    ...
    class PaymentUrl      
      ...
      def params_with_signature(params)    
        ...
        formatted_params = params.sort_by { |key, _| key.to_s }.map do |key, value|
          formatted_key = key.to_s.split('_').map.with_index { |word, i| 
            i == 0 ? word : word.capitalize
          }.join
          [formatted_key, value]
        end.to_h
      
        formatted_params['signature'] = Utils::Signature.generate(formatted_params.map { |k,v| "#{k}=#{v}" }.join('&'), PayOS.configuration.checksum_secret)
